<#
    One-shot: sync vault -> content/, rebuild, optionally commit + push.
    Usage:
      .\publish.ps1                  # sync + local build only
      .\publish.ps1 -Push            # sync + commit + push (triggers GitHub Pages)
...
if ($LASTEXITCODE -ne 0 -and $null -ne $LASTEXITCODE) { throw "sync-content.ps1 failed" }
